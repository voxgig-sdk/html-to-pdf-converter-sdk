...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HTML_TO_PDF_CONVERTER_TEST_LIVE") == "TRUE" ||
		os.Getenv("HTML_TO_PDF_CONVERTER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HTML_TO_PDF_CONVERTER_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.HtmlToPdfConverterSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.HtmlToPdfConverterSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.HtmlToPdfConverterError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.HtmlToPdfConverterSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.HtmlToPdfConverterError{Msg: msg, Code: code}
}
