...
func TestPdfGenerationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		pdfGenerationRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "pdf_generation"}), "pdf_generation_ref01"))

		pdfGenerationRef01DataResult, err := pdfGenerationRef01Ent.Create(pdfGenerationRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		pdfGenerationRef01Data = core.ToMapAny(entityData(pdfGenerationRef01DataResult))
...
func pdf_generationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HTML_TO_PDF_CONVERTER_TEST_PDF_GENERATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HTML_TO_PDF_CONVERTER_TEST_PDF_GENERATION_ENTID"])
