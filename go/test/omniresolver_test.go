...
import (
	...
	omni "github.com/voxgig-sdk/html-to-pdf-converter-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HtmlToPdfConverterError); is {
				out := map[string]any{
					"name":    "HtmlToPdfConverterError",
...
func omniCtx(arg any, client *sdk.HtmlToPdfConverterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HtmlToPdfConverterSDK); is {
			client = live
