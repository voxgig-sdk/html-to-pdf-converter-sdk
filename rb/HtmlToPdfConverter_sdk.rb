...
require_relative 'HtmlToPdfConverter_types'
...
class HtmlToPdfConverterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = HtmlToPdfConverterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, HtmlToPdfConverterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    HtmlToPdfConverterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => HtmlToPdfConverterError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue HtmlToPdfConverterError => err
    ...
    if fetched.is_a?(Hash)
      status = HtmlToPdfConverterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = HtmlToPdfConverterError.new(
        "graphql_error", "HtmlToPdfConverterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = HtmlToPdfConverterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
