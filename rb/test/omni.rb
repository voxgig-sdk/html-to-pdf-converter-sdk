...
module HtmlToPdfConverterOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if HtmlToPdfConverterOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = HtmlToPdfConverterOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| HtmlToPdfConverterOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || HtmlToPdfConverterOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = HtmlToPdfConverterSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = HtmlToPdfConverterError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { HtmlToPdfConverterOmni.errify(err) }
