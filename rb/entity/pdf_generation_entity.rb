...
class PdfGenerationEntity
  ...
  def make
    opts = @_entopts.dup
    PdfGenerationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PdfGeneration, Hash] the current PdfGeneration data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PdfGeneration fields)
  def match_get
  ...
  #
  # @param reqdata [PdfGenerationCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [PdfGeneration, Hash] the created PdfGeneration; raises HtmlToPdfConverterError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = HtmlToPdfConverterHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
