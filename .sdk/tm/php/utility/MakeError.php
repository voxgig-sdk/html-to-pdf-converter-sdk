...
// HtmlToPdfConverter SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class HtmlToPdfConverterMakeError
{
    public static function call(?HtmlToPdfConverterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HtmlToPdfConverterContext([], null);
        }
        $op = $ctx->op ?? new HtmlToPdfConverterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HtmlToPdfConverterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HtmlToPdfConverterError) ? $err->msg : (string)$err;
        $msg = "HtmlToPdfConverterSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new HtmlToPdfConverterError('', $msg, $ctx);
        ...
        if ($err instanceof HtmlToPdfConverterError) {
