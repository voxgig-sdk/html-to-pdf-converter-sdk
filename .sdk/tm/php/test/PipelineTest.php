...
// HtmlToPdfConverter SDK pipeline test
//
...
{
    private static function utility(): HtmlToPdfConverterUtility
    {
        return new HtmlToPdfConverterUtility();
    }
    ...
    private static function ctx(array $over = []): HtmlToPdfConverterContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new HtmlToPdfConverterContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new HtmlToPdfConverterOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof HtmlToPdfConverterError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new HtmlToPdfConverterOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = HtmlToPdfConverterMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new HtmlToPdfConverterOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = HtmlToPdfConverterMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new HtmlToPdfConverterOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = HtmlToPdfConverterMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HtmlToPdfConverterSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = HtmlToPdfConverterMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new HtmlToPdfConverterResponse([]),
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        [, $err] = HtmlToPdfConverterMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new HtmlToPdfConverterSpec([]),
            'response' => null,
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        [, $err] = HtmlToPdfConverterMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new HtmlToPdfConverterSpec(['step' => 's']),
            'response' => new HtmlToPdfConverterResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        [, $err] = HtmlToPdfConverterMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new HtmlToPdfConverterSpec(['step' => 's']),
            'response' => new HtmlToPdfConverterResponse(self::resp(200, ['v' => 1])),
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        [, $err] = HtmlToPdfConverterMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new HtmlToPdfConverterSpec(['step' => 's']),
            'response' => new HtmlToPdfConverterResponse(self::resp(200, ['v' => 2])),
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        HtmlToPdfConverterMakeResponse::call($ctx);
    ...
    {
        $preset = new HtmlToPdfConverterResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new HtmlToPdfConverterSpec([]),
            'response' => new HtmlToPdfConverterResponse([]),
            'result' => new HtmlToPdfConverterResult([]),
        ...
        [$got, $err] = HtmlToPdfConverterMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new HtmlToPdfConverterResult([])]);
        [, $err] = HtmlToPdfConverterMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new HtmlToPdfConverterSpec([]), 'result' => null]);
        [, $err] = HtmlToPdfConverterMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HtmlToPdfConverterSpec(['step' => 's']),
            'result' => new HtmlToPdfConverterResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new HtmlToPdfConverterOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HtmlToPdfConverterMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HtmlToPdfConverterSpec(['step' => 's']),
            'result' => new HtmlToPdfConverterResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new HtmlToPdfConverterOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HtmlToPdfConverterMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HtmlToPdfConverterResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new HtmlToPdfConverterSpec([]),
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = HtmlToPdfConverterMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HtmlToPdfConverterMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new HtmlToPdfConverterError('boom', 'boom');
        $utility->fetcher = function (HtmlToPdfConverterContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HtmlToPdfConverterSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = HtmlToPdfConverterMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (HtmlToPdfConverterContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = HtmlToPdfConverterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(HtmlToPdfConverterResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        HtmlToPdfConverterMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (HtmlToPdfConverterContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HtmlToPdfConverterSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = HtmlToPdfConverterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new HtmlToPdfConverterResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new HtmlToPdfConverterSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = HtmlToPdfConverterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = HtmlToPdfConverterMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = HtmlToPdfConverterMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new HtmlToPdfConverterResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, HtmlToPdfConverterDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new HtmlToPdfConverterResult(['ok' => false])]);
        $this->expectException(HtmlToPdfConverterError::class);
        HtmlToPdfConverterDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new HtmlToPdfConverterResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', HtmlToPdfConverterMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        HtmlToPdfConverterMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new HtmlToPdfConverterBaseFeature();
        $b = new HtmlToPdfConverterBaseFeature();
        HtmlToPdfConverterFeatureAdd::call($ctx, $a);
        HtmlToPdfConverterFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): HtmlToPdfConverterBaseFeature
    {
        $f = new HtmlToPdfConverterBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        HtmlToPdfConverterFeatureAdd::call($ctx, self::named_feature('a'));
        HtmlToPdfConverterFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        HtmlToPdfConverterFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        HtmlToPdfConverterFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        HtmlToPdfConverterFeatureAdd::call($ctx, $replace);
        ...
        HtmlToPdfConverterFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): HtmlToPdfConverterContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new HtmlToPdfConverterSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HtmlToPdfConverterPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        HtmlToPdfConverterPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new HtmlToPdfConverterResponse(['headers' => null]),
            'result' => new HtmlToPdfConverterResult([]),
        ]);
        HtmlToPdfConverterResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        HtmlToPdfConverterResultBody::call($ctx);
