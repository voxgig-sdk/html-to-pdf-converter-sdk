...
module HtmlToPdfConverterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HtmlToPdfConverterResponse.new({})
    ...
    if fetch_err
      ...
      response = HtmlToPdfConverterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HtmlToPdfConverterResponse.new(fetched)
    else
