...
//     (\`new HtmlToPdfConverterSDK(...)\` / \`HtmlToPdfConverterSDK.test(...)\` become
//     \`HtmlToPdfConverterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HtmlToPdfConverterSDK'
...
const TEST_SEED = {"entity":{"pdf_generation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HtmlToPdfConverterSDK(...)\` and any
// \`HtmlToPdfConverterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HtmlToPdfConverterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HtmlToPdfConverterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
