...
local PdfGenerationEntity = {}
PdfGenerationEntity.__index = PdfGenerationEntity


function PdfGenerationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PdfGenerationEntity)
  self._name = "pdf_generation"
  self._client = client
...
function PdfGenerationEntity:get_name()
  return self._name
end


function PdfGenerationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PdfGenerationEntity.new(self._client, opts)
end
...
function PdfGenerationEntity:mark_deleted()
  self._deleted = true
end


function PdfGenerationEntity:deleted()
  return true == self._deleted
end


function PdfGenerationEntity:data_set(args)
  if args ~= nil then
...
function PdfGenerationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PdfGenerationEntity:match_set(args)
  if args ~= nil then
...
function PdfGenerationEntity:match_get()
...
function PdfGenerationEntity:stream(action, args, callopts)
...
function PdfGenerationEntity:_run_op(ctx, post_done)
  local utility = self._utility
