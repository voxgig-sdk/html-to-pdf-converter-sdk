# HtmlToPdfConverter SDK test runner

from __future__ import annotations
import os
import json

from htmltopdfconverter_sdk.utility.voxgig_struct import voxgig_struct as vs


class HtmlToPdfConverterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HtmlToPdfConverterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HtmlToPdfConverterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HtmlToPdfConverterTestRunner.getenv("HTML_TO_PDF_CONVERTER_TEST_LIVE")
        override = HtmlToPdfConverterTestRunner.getenv("HTML_TO_PDF_CONVERTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HtmlToPdfConverterTestRunner.getenv(key)
        ...
        explain = HtmlToPdfConverterTestRunner.getenv("HTML_TO_PDF_CONVERTER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HTML_TO_PDF_CONVERTER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HtmlToPdfConverterTestRunner._test_control is not None:
            return HtmlToPdfConverterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HtmlToPdfConverterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HtmlToPdfConverterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HtmlToPdfConverterTestRunner.load_test_control()
...
def load_env_local():
    HtmlToPdfConverterTestRunner.load_env_local()


def env_override(m):
    return HtmlToPdfConverterTestRunner.env_override(m)


def entity_data(v):
    return HtmlToPdfConverterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HtmlToPdfConverterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HtmlToPdfConverterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HtmlToPdfConverterTestRunner.load_test_control()


def live_client_options():
    return HtmlToPdfConverterTestRunner.live_client_options()


def live_delay_ms():
    return HtmlToPdfConverterTestRunner.live_delay_ms()
