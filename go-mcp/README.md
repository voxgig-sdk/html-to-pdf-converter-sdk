# html-to-pdf-converter-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HtmlToPdfConverter SDK as
two agent tools — `html-to-pdf-converter_list` and `html-to-pdf-converter_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/html-to-pdf-converter-mcp)
make build

# 2. Provide credentials via the environment
export HTML_TO_PDF_CONVERTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user html-to-pdf-converter \
  -- /absolute/path/to/html-to-pdf-converter-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./html-to-pdf-converter-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// html-to-pdf-converter_list: first page of records
{ "entity": "pdf_generation" }
{ "entity": "pdf_generation", "query": { } }

// html-to-pdf-converter_load: one record by id
{ "entity": "pdf_generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/html-to-pdf-converter-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user html-to-pdf-converter \
     -- "$PWD"/dist/*/html-to-pdf-converter-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `html-to-pdf-converter_list` and `html-to-pdf-converter_load` tools now appear
   in new sessions. Ask the agent to *"list pdf_generation using html-to-pdf-converter"*
   and it calls `html-to-pdf-converter_list` with `{"entity":"pdf_generation"}`.
...
```sh
export HTML_TO_PDF_CONVERTER_APIKEY=sk_live_xxx            # API key
export HTML_TO_PDF_CONVERTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `html-to-pdf-converter_list` tool
...
### Call the `html-to-pdf-converter_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `html-to-pdf-converter_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `html-to-pdf-converter_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HTML_TO_PDF_CONVERTER_APIKEY` | API key sent with every request. |
| `HTML_TO_PDF_CONVERTER_BASE` | Optional override of the API base URL. |
...
```sh
./html-to-pdf-converter-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"html-to-pdf-converter_load","arguments":{"entity":"pdf_generation","query":{"id":1}}}}'
```
