...
// HtmlToPdfConverter SDK retry feature
...
class HtmlToPdfConverterRetryFeature extends HtmlToPdfConverterBaseFeature
{
    ...
    public function init(HtmlToPdfConverterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HtmlToPdfConverterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HtmlToPdfConverterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HtmlToPdfConverterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
