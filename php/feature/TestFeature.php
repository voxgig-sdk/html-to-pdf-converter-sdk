...
// HtmlToPdfConverter SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HtmlToPdfConverterTestFeature extends HtmlToPdfConverterBaseFeature
{
    ...
    public function init(HtmlToPdfConverterContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HtmlToPdfConverterContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HtmlToPdfConverterParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HtmlToPdfConverterContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HtmlToPdfConverterContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HtmlToPdfConverterParam::call($ctx, $k);
