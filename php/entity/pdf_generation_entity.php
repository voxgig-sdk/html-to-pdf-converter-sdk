...
// HtmlToPdfConverter SDK PdfGeneration entity
...
class PdfGenerationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PdfGenerationEntity($this->_client, $opts);
    }

    /**
     * @param PdfGeneration|array $args PdfGeneration data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HtmlToPdfConverterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PdfGeneration|array The current PdfGeneration data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PdfGeneration fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HtmlToPdfConverterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PdfGeneration fields).
     */
    ...
    /**
     * Create a new PdfGeneration.
     *
     * @param PdfGenerationCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed PdfGenerationCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PdfGeneration|array The created PdfGeneration as an assoc-array at the
     *   SDK boundary; throws HtmlToPdfConverterError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = HtmlToPdfConverterHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
