...
class HtmlToPdfConverterSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = HtmlToPdfConverterHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = HtmlToPdfConverterHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!HtmlToPdfConverterFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, HtmlToPdfConverterFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return HtmlToPdfConverterUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = HtmlToPdfConverterHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = HtmlToPdfConverterHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = HtmlToPdfConverterHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new HtmlToPdfConverterError($op . "_allow",
                "HtmlToPdfConverterSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = HtmlToPdfConverterHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new HtmlToPdfConverterError("graphql_error",
                "HtmlToPdfConverterSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function PdfGeneration($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_pdf_generation === null) {
                $this->_pdf_generation = new PdfGenerationEntity($this, null);
            }
            return $this->_pdf_generation;
        }
        return new PdfGenerationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new HtmlToPdfConverterSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
