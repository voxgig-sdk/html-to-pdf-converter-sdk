...
// HtmlToPdfConverter SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HtmlToPdfConverterOmni::makeStructRunner(
    ...
    {
        return HtmlToPdfConverterOmni::objmodel(HtmlToPdfConverterOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return HtmlToPdfConverterOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return HtmlToPdfConverterOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HtmlToPdfConverterOmni::structfix($now, Struct::undef()))
                        !== json_encode(HtmlToPdfConverterOmni::structfix($val, Struct::undef()))) {
