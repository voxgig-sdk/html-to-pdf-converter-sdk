...
//    only - a typed HtmlToPdfConverterContext there would make every ctx
...
final class HtmlToPdfConverterOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HtmlToPdfConverterContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HtmlToPdfConverterSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HtmlToPdfConverterResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HtmlToPdfConverterError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HtmlToPdfConverterResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HtmlToPdfConverterError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HtmlToPdfConverterError(is_string($code) ? $code : '', $msg);
    }
