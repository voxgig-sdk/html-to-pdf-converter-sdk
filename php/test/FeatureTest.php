...
// HtmlToPdfConverter SDK feature test
//
...
class FtCtrl extends HtmlToPdfConverterControl
{
...
class FtClient
{
    ...
    private HtmlToPdfConverterUtility $utility;

    public function __construct(HtmlToPdfConverterUtility $utility, array $options)
    {
    ...
    public function get_utility(): HtmlToPdfConverterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HtmlToPdfConverterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HtmlToPdfConverterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HtmlToPdfConverterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HtmlToPdfConverterConfig::make_feature($name);
    ...
    private static function buildUrl(HtmlToPdfConverterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HtmlToPdfConverterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HtmlToPdfConverterResponse($fetched);
        }
    ...
    private function fail(HtmlToPdfConverterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HtmlToPdfConverterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
