...
boru-driven command-line client **and** interactive REPL for the HtmlToPdfConverter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/html-to-pdf-converter-cli)
make build

# 2. See usage (words, entities, env vars)
./html-to-pdf-converter-cli --help
...
HTML_TO_PDF_CONVERTER_BASE=https://api.example.com ./html-to-pdf-converter-cli --help

# 6. No arguments -> interactive REPL
./html-to-pdf-converter-cli
html-to-pdf-converter> /help
html-to-pdf-converter> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/html-to-pdf-converter-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/html-to-pdf-converter-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `html-to-pdf-converter>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `pdf_generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/html-to-pdf-converter-cli`. |
...
(`./html-to-pdf-converter-cli <expr>`) and interactively (the REPL), and expressions compose the
