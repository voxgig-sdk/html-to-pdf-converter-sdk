// Command HtmlToPdfConverter-cli is a CLI + REPL for the HtmlToPdfConverter SDK,
...
const prompt = "html-to-pdf-converter"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "pdf_generation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HTML_TO_PDF_CONVERTER_APIKEY (api key), HTML_TO_PDF_CONVERTER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HTML_TO_PDF_CONVERTER_APIKEY carries the API key and
	// HTML_TO_PDF_CONVERTER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HTML_TO_PDF_CONVERTER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HTML_TO_PDF_CONVERTER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHtmlToPdfConverterSDK(opts)
